...
import { FriendRequest, User } from 'src/types/user.type'
...
export default function Header() {
  ...
  const { data, isFetching } = useQuery({
    ...
    queryFn: () =>
      notificationApi
        .getNotifications(profile?.phone || '', { page: currentPage, limit: itemsPerPage })
        .then((res) => res.data),
  ...
  const totalPages = notifications.length || 1
...
if (profile.role === 'ADMIN') {
  // Chuyển trang sang /admin-dashboard
  window.location.href = '/admin-dashboard' // Hoặc window.location.assign('/admin-dashboard')
} else {
  ...
  const handleClickNotification = async (purchase: Notification) => {
    try {
      ...
      if (purchase.type === 'friend_accept' || purchase.type === 'friend_remove') {
        navigate('/profile/' + purchase.sender + '/false')
      }
    } catch (error) {
        ...
        <div className='flex items-center  lg:justify-between justify-end'>
          ...
          <div className='flex items-center gap-4 '>
            <Link to='/' className='text-orange font-bold text-xl'>
              Web Social - {profileDataLS?.name}
            </Link>
            <button
              onClick={() => setIsMenuOpen(!isMenuOpen)}
              className='lg:hidden text-blue-500 bg-gray-200 cursor-pointer focus:outline-none px-2 py-1 rounded'
            >
          ...
          <div className={`lg:flex ${isMenuOpen ? 'block' : 'hidden'} mt-4`}>
            <div className='flex flex-col lg:flex-row items-start lg:items-center gap-4 text-orange'>
              <button onClick={handleAdmin}>Trang Admin</button>
              {/* Language */}
              <Popover
                renderPopover={
                      ...
                      <div className='p-2'>
                        ...
                        {notifications.map((n) => (
                          <div key={n._id} className='flex gap-2 p-2 hover:bg-gray-100 bg-gray-50 rounded mb-2'>
                            ...
                            <div className='flex-1'>
                              ...
                              <div className='text-xs text-gray-400'>{moment(n.timestamp).calendar()}</div>
                              {n.status === 'unread' && (
                        ...
                        <div className='flex justify-between mt-3'>
                          <button
                            ...
                            className={`px-2 py-1 text-xs rounded ${currentPage === 1 ? 'bg-gray-300' : 'bg-blue-500 text-white'
                              }`}
                          >
                            Trước
                          </button>
                          ...
                          <button
                            ...
                            onClick={() => setCurrentPage((p) => p + 1)}
                            className={`px-2 py-1 text-xs rounded ${currentPage === totalPages ? 'bg-gray-300' : 'bg-blue-500 text-white'
                              }`}
                          >
                            Sau
                          </button>
              ...
              {isAuthenticated ? (
                  ...
                  renderPopover={
                    <div className='rounded-sm border border-gray-200 bg-white shadow-md'>
                      ...
                      <button
                        ...
                        className='block px-4 py-2 hover:text-orange w-full text-left'
                      >
                        Đăng Xuất
                      </button>
                    </div>
                ...
                <div className='flex gap-3'>
                  <Link to={path.register} className='hover:text-orange'>
                    Register
                  </Link>
                  <Link to={path.login} className='hover:text-orange'>
                    Login
                  </Link>
                </div>
              )}
