...
export default function UserList() {
  ...
  const { data: productsData, refetch: refetchProducts } = useQuery({
    queryKey: ['products', queryConfig, selectedCategory],
    queryFn: () => {
    ...
    staleTime: 0 // set về 0 để luôn fetch khi refetch
  })
  ...
  useEffect(() => {
    if (selectedCategory === '3') {
      refetch() // Gọi lại API ngay khi đổi sang 3
    }
