...
export const BlockfriendUserService = async (userPhone: string, friendPhone: string): Promise<void> => {
  const existingFriendship = await AppDataSource.getRepository(Friend)
    .createQueryBuilder('f')
    .where(
      '(f.user_phone = :userPhone AND f.friend_phone = :friendPhone) OR (f.user_phone = :friendPhone AND f.friend_phone = :userPhone)',
  ...
  if (!existingFriendship) {
    const user = await AppDataSource.getRepository(User).findOne({ where: { phone: userPhone } });
    const friend = await AppDataSource.getRepository(User).findOne({ where: { phone: friendPhone } });
    // Check if both user and friend exist
    if (!user || !friend) {
      throw new Error('Không tìm thấy một hoặc cả hai người dùng.');
    }
    ...
    friendEntity.user = user;  // Assign the user
    friendEntity.friend = friend;  // Assign the friend
    friendEntity.user_phone = userPhone;
    friendEntity.friend_phone = friendPhone;

    friendEntity.status = "blocked"; // Example status

    await AppDataSource.getRepository(Friend).save(friendEntity);
  } else {
    // Nếu có thì cập nhật status = blocked
    existingFriendship.status = 'blocked';
    await AppDataSource.getRepository(Friend).save(existingFriendship);
  }
};
...
export const updateFriendStatusService = async (
  ...
  const existingFriendship = await repo
    .createQueryBuilder('f')
    .where(
      '(f.user_phone = :userPhone AND f.friend_phone = :friendPhone) OR (f.user_phone = :friendPhone AND f.friend_phone = :userPhone)',
      { userPhone, friendPhone }
  ...
  if (!existingFriendship) {
    throw new Error('No existing friend relationship found to update');
  }

  existingFriendship.status = status;
  await repo.save(existingFriendship);
};
