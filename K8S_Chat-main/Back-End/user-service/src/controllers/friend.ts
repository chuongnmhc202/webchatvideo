...
import {
  ...
  export const unfriendUserController = async (req: Request, res: Response) => {
    ...
    export const blockfriendUserController = async (req: Request, res: Response) => {
    try {
      const { senderPhone, receiverPhone } = req.body
      ...
      await BlockfriendUserService(senderPhone, receiverPhone)
  
      return res.status(200).json('Block thành công.')
    } catch (error) {
      console.error('Lỗi khi hủy kết bạn:', error)
      return res.status(500).json('Block thất bại.')
    }
  }
...
export const updateFriendStatusController = async (req: Request, res: Response) => {
  try {
    const { userPhone, friendPhone, status } = req.body;

    if (!userPhone || !friendPhone || !status) {
      return res.status(400).json({ message: 'Missing required fields' });
    }

    await updateFriendStatusService(userPhone, friendPhone, status);

    res.json({ message: 'Friend status updated successfully' });
  } catch (err: any) {
